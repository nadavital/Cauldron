...
final class RecipeCloudServiceRecordMappingTests: XCTestCase {
    ...
    func testPopulateAndDecodeRecipeRecordPreservesSourceLineageAndPreviewState() async throws {
        ...
        XCTAssertFalse(decodedIsFavorite)
    ...
    func testPopulatePublicRecipeRecordOmitsUserPrivateFavoriteState() async {
        let service = RecipeCloudService(core: CloudKitCore())
        let ownerId = UUID()
        let record = CKRecord(
            recordType: CloudKitCore.RecordType.sharedRecipe,
            recordID: CKRecord.ID(recordName: "public-recipe")
        )
        let recipe = Recipe(
            id: UUID(),
            title: "Favorite Source Recipe",
            ingredients: [],
            steps: [],
            yields: "4 servings",
            tags: [],
        ...
        await service.populateRecipeRecord(
            record,
            from: recipe,
            ownerId: ownerId,
            includesUserPrivateState: false
        )

        XCTAssertEqual(record["isFavorite"] as? Bool, false)
    }

    func testPopulatePrivateRecipeRecordPreservesUserFavoriteStateByDefault() async {
        let service = RecipeCloudService(core: CloudKitCore())
        let ownerId = UUID()
        let record = CKRecord(
            recordType: CloudKitCore.RecordType.recipe,
            recordID: CKRecord.ID(recordName: "private-recipe")
        )
        let recipe = Recipe(
            id: UUID(),
            title: "Favorite Private Recipe",
            ingredients: [],
            steps: [],
            yields: "4 servings",
            tags: [],
        ...
        XCTAssertEqual(record["isFavorite"] as? Bool, true)
    }
