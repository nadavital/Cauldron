...
- Social sharing is a core workflow:
  ...
  - Recipe share links use canonical `/recipe/{id}` URLs. A successful publication receipt lets an unchanged recipe share immediately; new or edited revisions still wait for server confirmation. Open Graph artwork uses the stable same-origin `/recipe/{id}/social-card.png` proxy with the branded static card as its fallback, never a signed CloudKit asset URL.
