...
struct TimerSpec: Sendable, Hashable, Identifiable {
    let id: UUID
    let seconds: Int
    let label: String
    
    nonisolated init(id: UUID = UUID(), seconds: Int, label: String = "Timer") {
        self.id = id
        self.seconds = max(0, seconds)
        self.label = label
    }
    
    nonisolated var displayDuration: String {
    ...
    nonisolated static func minutes(_ minutes: Int, label: String = "Timer") -> TimerSpec {
        TimerSpec(seconds: minutes * 60, label: label)
    }
    
    /// Create a TimerSpec from hours
    nonisolated static func hours(_ hours: Int, label: String = "Timer") -> TimerSpec {
