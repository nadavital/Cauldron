...
- [ ] Implement Milestone 1: Define dataset contract, collect seed corpus, and add data validation tooling.
- [ ] Implement Milestone 2: Train baseline line-classification model and establish acceptance thresholds.
- [ ] Implement Milestone 3: Export and bundle Core ML model, add on-device inference service, and wire parser integration.
- [ ] Implement Milestone 4: Add confidence policy, regression suite, and repeatable retraining workflow.
...
This plan is authored but not implemented yet. The expected outcome is a fully local, bundled, testable import pipeline that improves section separation quality without introducing any network dependency.
